...
**Status:** required for useful Mobile IO · **Runtime:** Redroid only · **Cuttlefish:** out of scope
...
| Image | GApps |
|---|---|
| Redroid containers (`./cloud-phone redroid-up`) | **Install here** |
| Cuttlefish golden (RTMP / v4l2 / camera HAL) | **Do not install** — ingest-only |
...
Do **not** commit proprietary zips. Do **not** trust the historical SourceForge/GitHub URLs in `0028cb4:scripts/install-gapps.sh` — they 404'd and produced the empty zip.
...
Control API `GET /health` reports `gapps: { gms, play_store, gsf }` when ADB is connected.
